...
export default function ProfilePage() {
  ...
  const featureIcons: FeatureIcon[] = [
    { icon: WalletIcon, label: "钱包", color: "text-green-500" },
    { icon: Gift, label: "线下活动", color: "text-blue-500" },
    { icon: Receipt, label: "发票中心", color: "text-yellow-500" },
    { icon: UserRoundPlus, label: "邀请码", color: "text-red-500" },
    { icon: IdCard, label: "实名认证", color: "text-purple-500", badge: !userInfo.isVerified ? "未实名" : undefined },
  ];
    ...
    },
    {
      ...
      content: (
        <div className="space-y-2 text-sm text-muted-foreground">
          <p className="font-medium text-foreground">个性化装扮</p>
          <div className="grid grid-cols-3 gap-2">
            <div className="aspect-square bg-muted rounded-lg flex items-center justify-center">
              <span className="text-xs text-foreground">头像框</span>
            </div>
            <div className="aspect-square bg-muted rounded-lg flex items-center justify-center">
              <span className="text-xs text-foreground">背景</span>
            </div>
            <div className="aspect-square bg-muted rounded-lg flex items-center justify-center">
              <span className="text-xs text-foreground">徽章</span>
            </div>
          </div>
    ...
    },
    {
      ...
      content: (
        <div className="space-y-3 text-sm text-muted-foreground">
          <div>
            <p className="font-medium text-foreground mb-1">Q: 如何升级会员等级？</p>
            <p className="text-xs">A: 通过消费累积积分，达到相应积分阈值即可自动升级。</p>
          </div>
          <div>
            <p className="font-medium text-foreground mb-1">Q: 积分如何获得？</p>
            <p className="text-xs">A: 每次消费均可获得积分，具体规则请查看积分说明。</p>
          </div>
          <div>
            <p className="font-medium text-foreground mb-1">Q: 积分会过期吗？</p>
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="fixed right-4 bottom-24 z-30">
        <Button className="bg-primary hover:bg-primary/90 text-primary-foreground rounded-full px-4 py-2 shadow-lg">
